...
Use one record for supplier-side change, claim, and compliance history. Separate registers are justified only if legal or audit requirements specifically demand them. Most projects can operate with a single integrated log that distinguishes event types by field entry rather than by file.

---

## Waste Test

**Fail if:** Change orders, claims logs, and supplier acceptance checklists are maintained as separate files with the same case identifiers and decision fields. Duplication creates reconciliation overhead with no added control value.
...
|---|---|
| A31 Procurement and Supplier Management Record | Planning and ongoing administration — A33 captures exceptions within that context |
