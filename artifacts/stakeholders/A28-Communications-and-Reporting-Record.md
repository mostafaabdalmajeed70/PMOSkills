...
exact_section: "Guide §2.5.2.3 Plan Communications Management; §2.5.2.5 Monitor Communications; Table 2-1 Plan Communications Management, Manage Communications, Monitor Communications; Figure 2-35"
performance_domains:
  - Stakeholders
  - Governance
focus_areas:
...
| When used | During Planning (communications plan section); maintained and updated through Executing and Monitoring and Controlling. |
...
| Purpose of citation | Ground communications planning and management in PMBOK 8 stakeholder performance domain and the three communications processes. |
...
| Inputs | Stakeholder register and engagement strategy (A07); project schedule (A15); tailoring record (A06); reporting cadence from A14 |
...
| Owner | Project manager |
| Approval authority | Project manager; sponsor or governance body for high-sensitivity, external, or confidential communications |
| Tailoring guidance | Every stakeholder in A07 must have at least one entry in A28. Scale the depth of each entry to the stakeholder's influence and information need. Do not produce a separate Communications Management Plan document. |
...
| Notes | Replaces v1.0.2 Items 190–192 and 211. A28 is built by SKL-03-08. Every A28 entry must reference an A07 stakeholder — orphan entries are not permitted (enforced by SKL-03-08 Test T-3). |
...
1. A28 is the single communications record — no separate communications management plan document is permitted.
...
4. Every entry must specify all three communication methods where applicable (interactive, push, pull); push-only plans are flagged as incomplete.
5. Every entry must have a feedback route — communications with no feedback mechanism are flagged as incomplete.
6. Cross-cultural stakeholder entries must reference the PMBOK8 communication model (Figure 5-2) in the approach section.
7. Communication events that precede key decisions must be scheduled in A15 and cross-referenced here.
8. Fail the waste test if a separate communications plan document, a reporting matrix spreadsheet, and a stakeholder update log each hold the same entries.
